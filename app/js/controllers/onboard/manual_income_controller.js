(function() {
  ...
  angular.module('onboard').controller('onboardManualIncomeCtrl', function($scope, $rootScope, $location, $q, $timeout, matchmedia, Income, onboardService, Person, welcomeTextService) {
      ...
      $scope.processForm = function(form) {
          var calls, updateIncomes;
          $scope.submitted = true;
          if (form.$valid) {
              $scope.pending = true;
              updateIncomes=[];
              updateIncomes = prepareIncomes($scope.incomes);
              if ($scope.isAnyOtherIncome)
              {
                  updateIncomes = updateIncomes.concat(prepareIncomes($scope.otherIncomes));
              }

              Income.updateList(updateIncomes).$promise.then(
                      function () {
